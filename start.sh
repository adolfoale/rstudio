...
# the efx_container_user user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the efx_container_user user to the desired the user (NB_USER)
    if id efx_container_user &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" efx_container_user 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the efx_container_user user:"
            _log "- username: efx_container_user       -> ${NB_USER}"
            _log "- home dir: /home/efx_container_user -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the efx_container_user user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the efx_container_user home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "efx_container_user" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/efx_container_user to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/efx_container_user/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/efx_container_user to /home/${NB_USER}!"
                _log "Attempting to symlink /home/efx_container_user to /home/${NB_USER}..."
                if ln -s /home/efx_container_user "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/efx_container_user to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/efx_container_user/"* ]]; then
...
# user we started as.
    ...
    JOVYAN_UID="$(id -u efx_container_user 2>/dev/null)"  # The default UID for the efx_container_user user
    JOVYAN_GID="$(id -g efx_container_user 2>/dev/null)"  # The default GID for the efx_container_user user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old efx_container_user user to nayvoj ($(id -u efx_container_user):$(id -g efx_container_user))"
            ...
            sed --expression="s/^efx_container_user:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/efx_container_user:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "efx_container_user" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/efx_container_user. You must run as root to rename the home directory!"
            fi
        else
    ...
    # are not root.
    if [[ "${NB_USER}" != "efx_container_user" && "${NB_USER}" != "$(id -un)" ]]; then
    ...
    if [[ ! -w /home/efx_container_user ]]; then
        _log "WARNING: no write access to /home/efx_container_user. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
